...
const CertiVault = () => {
  ...
  const renderEditor = () => {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row animate-in fade-in duration-500 pb-24 lg:pb-0">
        ...
        <div className="lg:flex-1 lg:h-screen lg:sticky lg:top-0 p-4 md:p-8 flex flex-col bg-slate-200/50">
          ...
          <div className="relative flex-1 bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden group flex items-center justify-center p-2">
            ...
            <button 
              ...
              className="absolute bottom-6 right-6 px-6 py-3 bg-slate-900/90 backdrop-blur-md text-white text-xs font-black rounded-2xl flex items-center gap-2 shadow-2xl hover:bg-indigo-600 transition-all active:scale-95 touch-none"
            >
              <Layers className="w-5 h-5" /> <span>Hold to Compare</span>
            </button>
          </div>
        </div>

        {/* Right/Bottom: Form Section */}
        <div className="w-full lg:w-[500px] xl:w-[600px] bg-white lg:h-screen overflow-y-auto shadow-[-20px_0_40px_rgba(0,0,0,0.02)] z-10 border-l border-slate-100 p-6 md:p-10">
          <div className="hidden lg:flex items-center gap-4 mb-10">
            <button onClick={() => setView('dashboard')} className="p-3 hover:bg-slate-50 rounded-2xl transition-colors">
              <ChevronLeft className="w-6 h-6" />
            </button>
            <div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">Data Verification</h2>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Retrieved via Gemini AI Vision</p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
              <div className="flex items-center gap-2 text-[11px] font-black text-indigo-500 uppercase tracking-widest">
                <UserIcon className="w-4 h-4" /> Destination Profile
              </div>
              {hasProfiles ? (
                <div className="relative group">
                  <select 
                    value={pendingCert.profileId || activeProfileId || (profiles[0]?.id)}
                    onChange={e => setPendingCert({ ...pendingCert, profileId: e.target.value })}
                    className="w-full p-4 bg-white border border-slate-200 rounded-2xl font-black text-slate-800 outline-none cursor-pointer appearance-none pr-10 focus:ring-2 focus:ring-indigo-500/20 transition-all shadow-sm"
                  >
                    {profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                ...
                <input 
                  type="text" 
                  placeholder="Create first profile name..." 
                  value={editorNewProfileName} 
                  onChange={e => setEditorNewProfileName(e.target.value)} 
                  className="w-full p-4 bg-white border border-indigo-200 rounded-2xl font-black text-slate-800 outline-none shadow-sm placeholder:text-slate-300" 
                />
              )}
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <FileText className="w-3.5 h-3.5" /> Certificate Title
                </label>
                <input 
                  type="text" 
                  value={pendingCert.title} 
                  onChange={e => setPendingCert({ ...pendingCert, title: e.target.value })} 
                  className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    <UserPlus className="w-3.5 h-3.5" /> Student Name
                  </label>
                  <input 
                    type="text" 
                    value={pendingCert.studentName} 
                    onChange={e => setPendingCert({ ...pendingCert, studentName: e.target.value })} 
                    className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                  />
                </div>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    <Building className="w-3.5 h-3.5" /> Issuing Body
                  </label>
                  <input 
                    type="text" 
                    value={pendingCert.issuer} 
                    onChange={e => setPendingCert({ ...pendingCert, issuer: e.target.value })} 
                    className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {isAddingCustomCategory ? (
                    <div className="flex items-center gap-2">
                      <input 
                        ...
                        className="flex-1 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl font-black outline-none" 
                      />
                      <button onClick={handleCreateCustomCategory} className="p-4 bg-indigo-600 text-white rounded-2xl shadow-lg">
                    ...
                    <div className="relative">
                      <select 
                        value={pendingCert.category} 
                        onChange={e => e.target.value === '__add_custom__' ? setIsAddingCustomCategory(true) : setPendingCert({ ...pendingCert, category: e.target.value })} 
                        className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-800 outline-none appearance-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all"
                      >
                        {categoriesList.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                        <option value="__add_custom__">+ Create New...</option>
                      </select>
                      <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                    </div>
                  )}
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    <Calendar className="w-3.5 h-3.5" /> Award Date
                  </label>
                  <input 
                    type="date" 
                    value={pendingCert.date} 
                    onChange={e => setPendingCert({ ...pendingCert, date: e.target.value })} 
                    className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <Hash className="w-3.5 h-3.5" /> Quick Tags
                </label>
                <div className="flex flex-wrap gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl min-h-[64px] items-center focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
                  {pendingCert.tags?.map(tag => (
                    <span key={tag} className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase shadow-sm animate-in zoom-in-90">
                      #{tag} 
                      <button onClick={() => removeTag(tag)} className="p-0.5 hover:bg-white/20 rounded-md transition-colors">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  ))}
                  <input 
                    ...
                    placeholder={pendingCert.tags?.length ? "" : "Add keywords..."}
                    className="flex-1 bg-transparent font-black text-slate-800 outline-none min-w-[150px] placeholder:text-slate-300"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <Info className="w-3.5 h-3.5" /> AI Observations
                </label>
                <textarea 
                  value={pendingCert.summary} 
                  onChange={e => setPendingCert({ ...pendingCert, summary: e.target.value })} 
                  className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl font-medium text-slate-600 outline-none min-h-[120px] resize-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all leading-relaxed" 
                />
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            disabled={!hasProfiles && !editorNewProfileName.trim()} 
            className="hidden lg:flex w-full mt-10 py-6 bg-slate-900 text-white rounded-[2rem] font-black text-xl shadow-2xl hover:bg-indigo-600 transition-all disabled:bg-slate-200 disabled:shadow-none items-center justify-center gap-4 group active:scale-[0.98]"
          >
        ...
        {/* Mobile Action Bar */}
        <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-xl border-t border-slate-100 flex gap-4 z-50">
          <button 
            onClick={() => setView('dashboard')}
            className="p-5 bg-slate-100 text-slate-600 rounded-2xl transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
          <button 
            onClick={saveCertificate} 
            disabled={!hasProfiles && !editorNewProfileName.trim()} 
            className="flex-1 py-5 bg-indigo-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-3 disabled:bg-slate-200"
          >
            <CheckCircle className="w-6 h-6" /> SAVE VAULT
          </button>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        <header className="bg-white px-8 py-5 border-b border-slate-100 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button onClick={() => shareSingleCertificate(selectedCert)} className="p-3 bg-white border border-slate-100 rounded-xl text-indigo-600 hover:bg-indigo-50 shadow-sm transition-colors"><Share2 className="w-5 h-5" /></button>
            <button onClick={() => syncSingleItem(selectedCert)} className="flex items-center gap-2 px-5 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-sm">{isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : (selectedCert.synced ? <CheckCircle className="w-4 h-4" /> : <Cloud className="w-4 h-4" />)} {selectedCert.synced ? 'Synced' : 'Sync to Drive'}</button>
            <button onClick={() => { if(confirm("Delete?")) { setCertificates(prev => prev.filter(c => c.id !== selectedCert.id)); setView('dashboard'); } }} className="p-3 text-red-400 hover:bg-red-50 rounded-xl transition-colors"><Trash2 className="w-5 h-5" /></button>
          </div>
        </header>
        <main className="max-w-6xl mx-auto w-full p-8 grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-12">
          <div className="bg-white p-6 rounded-3xl shadow-xl h-fit flex items-center justify-center">
            <img src={selectedCert.image} className="w-full h-auto rounded-2xl border border-slate-100 shadow-sm" />
          </div>
          <div className="space-y-8">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              <h2 className="text-3xl font-black text-slate-900 mb-2 leading-tight">{selectedCert.title}</h2>
              <p className="text-xl text-slate-400 font-bold mb-4">{selectedCert.issuer}</p>
              
              <div className="flex flex-wrap gap-2 mb-8">
                {selectedCert.tags.map(t => (
                  <span key={t} className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-black uppercase tracking-wider">#{t}</span>
                ))}
                <span className="px-3 py-1 bg-slate-50 text-slate-500 rounded-lg text-xs font-black uppercase tracking-wider">{selectedCert.category}</span>
                {selectedCert.synced && (
                  <span className="px-3 py-1 bg-green-50 text-green-600 rounded-lg text-[10px] font-black flex items-center gap-1 uppercase tracking-widest">
              ...
              <div className="grid grid-cols-2 gap-6 mb-8">
                <div>
                   ...
                   <p className="font-bold text-slate-800 flex items-center gap-2"><Calendar className="w-4 h-4 text-indigo-500" /> {selectedCert.date}</p>
                </div>
                <div>
                   <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Academic Year</h4>
                   <p className="font-bold text-slate-800 flex items-center gap-2"><Folder className="w-4 h-4 text-indigo-500" /> {getAcademicYear(selectedCert.date)}</p>
                </div>
              </div>

              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-8">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                  <Server className="w-3 h-3" /> Drive Path Info
                </h4>
                <p className="text-[11px] font-mono text-slate-500 leading-tight">
                  Drive: /CertiVault/{profiles.find(p => p.id === selectedCert.profileId)?.name || 'Default'}/{getAcademicYear(selectedCert.date)}/{selectedCert.category}/{selectedCert.title}.pdf
                </p>
              </div>

              <div className="space-y-2">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">AI Summary</h4>
                <p className="text-slate-600 leading-relaxed italic bg-slate-50 p-6 rounded-2xl border border-slate-100">"{selectedCert.summary}"</p>
              </div>
            </div>
            <button 
              onClick={() => shareSingleCertificate(selectedCert)}
              className="w-full py-5 bg-indigo-600 text-white rounded-3xl font-black text-lg shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-3"
            >
              <Share2 className="w-6 h-6" /> Share Full PDF Report
            </button>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#FDFEFE]">
      {/* Logic to show Login or Content */}
      {!user ? (
        renderLogin()
      ) : (
        <>
          {/* Processing & Sync Progress Overlays */}
          {(isProcessing || isSharing || isSyncing) && (
            <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-xl flex flex-col items-center justify-center text-white p-10 text-center overflow-hidden">
              {isSyncing ? (
                <div className="w-full max-w-lg bg-white/5 border border-white/10 p-8 rounded-[3rem] animate-in zoom-in-95 duration-300">
                  <div className="mb-8 flex justify-center">
                    <div className="relative">
                        <Cloud className="w-20 h-20 text-indigo-400 animate-pulse" />
                        <Loader2 className="w-24 h-24 text-white/10 animate-spin absolute inset-0 -translate-x-2 -translate-y-2" />
                    </div>
                  </div>
                  <h2 className="text-3xl font-black mb-6 text-white tracking-tight">Syncing Hierarchy</h2>
                  <div className="space-y-3 text-left h-[240px] overflow-y-auto no-scrollbar mask-fade">
                      {syncLogs.map((log, i) => (
                        <div key={log.id} className={`flex items-start gap-3 text-sm font-bold transition-all animate-in slide-in-from-left-4 duration-500`} style={{ animationDelay: `${i * 100}ms` }}>
                          {log.status === 'pending' ? <Loader2 className="w-4 h-4 animate-spin text-indigo-400 mt-0.5 shrink-0" /> : 
                          log.status === 'success' ? <CheckCircle className="w-4 h-4 text-green-400 mt-0.5 shrink-0" /> : 
                          <Server className="w-4 h-4 text-slate-400 mt-0.5 shrink-0" />}
                          <span className={`${log.status === 'success' ? 'text-green-400' : log.status === 'info' ? 'text-slate-400' : 'text-slate-200'}`}>
                            {log.message}
                          </span>
                        </div>
                      ))}
                ...
                <>
                  ...
                  <h2 className="text-3xl font-black mb-3 text-white">{isSharing ? 'Preparing PDF...' : 'AI Vision Processing'}</h2>
                  <p className="text-slate-400 text-lg">{isSharing ? 'Compiling certificates...' : 'Restoring certificate and scaling to A4 size...'}</p>
                </>
              )}
            </div>
          )}
          ...
          {error && (
            <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] px-6 py-4 bg-red-600 text-white rounded-2xl shadow-xl flex items-center gap-3 font-bold animate-in slide-in-from-top-4">
